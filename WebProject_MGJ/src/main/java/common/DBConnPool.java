 ...
 * 참조(Lookup)하는 API로 쉽게말하면 외부에 있는 객체를
 * 이름으로 찾아오기 위한 기술이다.
 * 
 * DBCP(DataBase Connection Pool : 커넥션풀)
 * : DB와 연결된 커넥션 객체를 미리 만들어 풀(Pool)에
 * 저장해 두었다가 필요할 때 가져다 쓰고 반납하는 기법을 말함.
...
public class DBConnPool {
	...
	public DBConnPool() {
		try {
			//1. Context 인스턴스를 생성한다. Tomcat웹서버라
			//생각하면 된다.
			Context initCtx = new InitialContext();
			
			/* 2. 앞에서 생성한 인스턴스를 통해 JNDI서비스 구조의
			 * 초기 Root디렉토리를 얻어온다. 해당 디렉토리의 이름은
			 * 아래와 같이 이미 지정되어있으므로 그대로 쓰면됨 */
